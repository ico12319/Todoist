...
func (s *sqlListDB) DeleteList(ctx context.Context, listID string) error {
	...
	if err != nil {
		log.C(ctx).Errorf("failed to delete list with id %s due to a failiure in the execution of the sql query")
		return err
	}
...
func (s *sqlListDB) GetLists(ctx context.Context, queryBuilder sqlQueryRetriever) ([]entities.List, error) {
...
func (s *sqlListDB) UpdateListSharedWith(ctx context.Context, listId string, userId string) error {
	...
	rowsAffected, err := res.RowsAffected()
	if err != nil {
		log.C(ctx).Errorf("failed to add collaborator to list with id %s, error when trying to get the number of rows affected")
		return err
	}
...
func (s *sqlListDB) GetListCollaborators(ctx context.Context, sqlQueryBuilder sqlQueryRetriever) ([]entities.User, error) {
